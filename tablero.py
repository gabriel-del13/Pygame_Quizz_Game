...
class Casilla:
    def __init__(self, x, y, id_casilla, numero_display, color, siguientes=None, tiene_pregunta=True, dificultad=None):
        self.x = x
        self.y = y
        self.id = id_casilla
    ...
    def dibujar(self, pantalla, offset_x, offset_y):
        ...
        # Número
        fuente = pygame.font.Font(None, 30 if self.numero == "BONUS" else 50)
        color_texto = BLANCO if self.color in (MARRON_OSCURO, AZUL) else NEGRO

        # Si está completada (Verde/Rojo) o es Meta, texto negro se lee mejor
        if self.completada or self.es_final:
            color_texto = NEGRO

        # Si hay dificultad, subir el número para hacer hueco al subtítulo
        if self.dificultad and not self.completada and not self.es_final:
            centro_y = screen_y + TAMANO_CASILLA // 2 - 14
        else:
            centro_y = screen_y + TAMANO_CASILLA // 2

        texto = fuente.render(str(self.numero), True, color_texto)
        texto_rect = texto.get_rect(center=(screen_x + TAMANO_CASILLA//2, centro_y))
        pantalla.blit(texto, texto_rect)

        # Subtítulo de dificultad
        if self.dificultad and not self.completada and not self.es_final:
            fuente_dif = pygame.font.Font(None, 24)
            dif_texto = fuente_dif.render(self.dificultad, True, color_texto)
            dif_rect = dif_texto.get_rect(center=(screen_x + TAMANO_CASILLA//2,
                                                  screen_y + TAMANO_CASILLA // 2 + 20))
            pantalla.blit(dif_texto, dif_rect)
...
class Tablero:
    ...
    def crear_tablero_con_bifurcaciones(self):
        ...
        # Casilla 1 (inicio)
        casillas.append(Casilla(x_izq, filas[0], "1", "1", AZUL, siguientes=["2"], tiene_pregunta=True, dificultad="FACIL"))
        print(f"Casilla 1: x={x_izq}, y={filas[0]} -> siguiente: 2")

        # Casilla 2
        casillas.append(Casilla(x_izq, filas[1], "2", "2", AZUL, siguientes=["3"], tiene_pregunta=True, dificultad="FACIL"))
        print(f"Casilla 2: x={x_izq}, y={filas[1]} -> siguiente: 3")

        # Casilla 3 (BIFURCACIÓN: puede ir a 4 o a 3.1)
        casillas.append(Casilla(x_izq, filas[2], "3", "3", AZUL, siguientes=["4", "3.1"], tiene_pregunta=True, dificultad="FACIL"))
        ...
        casillas.append(Casilla(x_extra_izq, filas[2], "3.1", "BONUS", ROJO_CLARO, siguientes=["3.2"], tiene_pregunta=True, dificultad="DIFICIL"))
        print(f"Casilla 3.1 (ALT): x={x_extra_izq}, y={filas[2]} -> siguiente: 3.2")

        # Casilla 3.2 (sube)
        casillas.append(Casilla(x_extra_izq, filas[3], "3.2", "BONUS", ROJO_CLARO, siguientes=["3.3"], tiene_pregunta=True, dificultad="DIFICIL"))
        print(f"Casilla 3.2 (ALT): x={x_extra_izq}, y={filas[3]} -> siguiente: 3.3")

        # Casilla 3.3 (sube más y se une al 9)
        casillas.append(Casilla(x_extra_izq, filas[4], "3.3", "BONUS", ROJO_CLARO, siguientes=["9"], tiene_pregunta=True, dificultad="DIFICIL"))
        ...
        casillas.append(Casilla(x_centro, filas[2], "4", "4", AZUL, siguientes=["5"], tiene_pregunta=True, dificultad="FACIL"))
        print(f"Casilla 4: x={x_centro}, y={filas[2]} -> siguiente: 5")

        # Casilla 5
        casillas.append(Casilla(x_der, filas[2], "5", "5", AMARILLO, siguientes=["6"], tiene_pregunta=True, dificultad="INTERMEDIO"))
        print(f"Casilla 5: x={x_der}, y={filas[2]} -> siguiente: 6")

        # Casilla 6
        casillas.append(Casilla(x_der, filas[3], "6", "6", AMARILLO, siguientes=["7"], tiene_pregunta=True, dificultad="INTERMEDIO"))
        print(f"Casilla 6: x={x_der}, y={filas[3]} -> siguiente: 7")

        # Casilla 7
        casillas.append(Casilla(x_der, filas[4], "7", "7", AMARILLO, siguientes=["8"], tiene_pregunta=True, dificultad="INTERMEDIO"))
        print(f"Casilla 7: x={x_der}, y={filas[4]} -> siguiente: 8")

        # Casilla 8
        casillas.append(Casilla(x_centro, filas[4], "8", "8", AMARILLO, siguientes=["9"], tiene_pregunta=True, dificultad="INTERMEDIO"))
        ...
        casillas.append(Casilla(x_izq, filas[4], "9", "9", ROJO_CLARO, siguientes=["10"], tiene_pregunta=True, dificultad="DIFICIL"))
        print(f"Casilla 9 (UNIÓN): x={x_izq}, y={filas[4]} -> siguiente: 10")

        # Casilla 10
        casillas.append(Casilla(x_izq, filas[5], "10", "10", ROJO_CLARO, siguientes=["META"], tiene_pregunta=True, dificultad="DIFICIL"))
